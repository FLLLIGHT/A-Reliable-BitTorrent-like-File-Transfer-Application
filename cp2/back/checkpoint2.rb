...
def test1
        ...
        peer1_pid = fork do
            exec("./ref_peer -p nodes.map -c A.chunks -f C.chunks -m 4 -i 1 -x 2 -d 63")
        end 
	...
	peer2_pid = fork do
	    ...
	    exec("./peer -p nodes.map -c B.chunks -f C.chunks -m 4 -i 2 -d 63")    
	end
...
def test2
        ...
        peer1_pid = fork do
            exec("./peer -p nodes.map -c B.chunks -f C.chunks -m 4 -i 1  -d 63")
        end 
	...
	peer2_pid = fork do
	    ...
	    exec("./ref_peer -p nodes.map -c A.chunks -f C.chunks -m 4 -i 2 -x 2 -d 63")    
	end
	parent_to_child_read.close
